...
public class RechargeServiceImpl implements IRechargeServiceInf {
    ...
    //富有充值返回
    public void fuiouRechargeNotify(Long loginId,FuiouRechargeRequest reqData,String respData, TbFundsDetails details){
        ...
        if(details != null){
             details.setFdSerialStatus(1);
             fundsDetailsService.update(details);
        }else{
            //todo 添加资金收支明细
            TbFundsDetails fundsDetails = new TbFundsDetails();
            fundsDetails.setFdBalanceBefore(balance);
